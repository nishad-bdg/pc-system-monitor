...
export type PrintJobsByPrinterRow = {
  printer: string;
  jobs: number;
  pages: number;
  pcs?: (string | null)[];
...
export type PrintJobsByPrinter = {
  ...
  total_jobs: number;
  total_pages: number;
  printers: PrintJobsByPrinterRow[];
};

export function fetchPrintJobsByPrinter(
  apiUrl: string,
  apiToken: string,
  filters?: {
    fromTs?: number;
